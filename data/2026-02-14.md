...
Motivation: 类星体光变曲线的时间延迟测量对于宇宙学参数约束至关重要，但现有方法通常假设光变曲线是平稳的，而实际观测中可能存在非平稳性（如长期漂移），这会影响时间延迟推断的准确性。需要开发能够同时检验平稳性和推断时间延迟的统计框架。

Method: 采用完全贝叶斯框架，将光变曲线分解为确定性非平稳漂移（分段线性均值函数）和平稳随机变化（使用Matérn核和谱混合核的高斯过程）。联合建模多个图像，包含每个图像的微引力透镜效应。通过嵌套采样获得贝叶斯证据和参数后验分布，并在模型选择上进行边缘化。识别并缓解了似然函数病理问题，提供了实用的嵌套采样收敛协议。

Result: 应用于WFI J2033-4723、B 1608+656和HE 0435-1223三个类星体：B 1608+656和HE 0435-1223显示出强烈的非平稳性证据，而WFI J2033-4723与平稳性一致。随机分量偏好：B 1608+656使用马尔可夫指数核，WFI J2033-4723和HE 0435-1223使用非马尔可夫Matérn-3/2核。多长度尺度谱混合核不被支持。时间延迟对模型假设具有鲁棒性，与先前工作结果在误差范围内一致。

Conclusion: 该框架能够同时进行平稳性检验和时间延迟推断，识别出类星体光变曲线中的非平稳性，并证明时间延迟估计对模型选择具有鲁棒性。提出的似然病理缓解方法和收敛协议提高了推断的可靠性，为精确宇宙学测量提供了改进的统计工具。
...
TL;DR: SPT-3G与Planck数据结合构建热Sunyaev-Zel'dovich康普顿-y参数图，通过线性组合方法平衡统计灵敏度与天体物理污染抑制，验证了在1500平方度巡天区域中tSZ信号的稳健恢复。


<details>
  <summary>Details</summary>
Motivation: 构建高分辨率、低噪声的热Sunyaev-Zel'dovich康普顿-y参数图，为宇宙学研究提供重要工具，包括测量康普顿-y图功率谱、与大尺度结构示踪物的交叉相关、星系团压力剖面详细建模以及宇宙重子热力学状态研究。

Method: 使用SPT-3G相机两年观测数据（2019-2020）与Planck卫星数据结合，采用线性组合（LC）管道构建康普顿-y参数图。探索不同统计灵敏度与天体物理污染抑制之间的权衡，包括最小方差、CMB去投影和CIB去投影的y图。

Result: 通过自功率谱和交叉功率谱与大尺度结构示踪物的统计分析，以及在星系团位置上的堆叠测试，验证了不同图中噪声与天体物理前景残差（如CIB）的平衡以及tSZ信号的恢复。在约1500平方度的SPT-3G巡天区域中，星系团位置堆叠结果证实了恢复的tSZ信号的稳健性。

Conclusion: 构建的高分辨率、低噪声康普顿-y参数图为未来宇宙学研究提供了重要工具，包括测量康普顿-y图功率谱、与大尺度结构示踪物的交叉相关、星系团压力剖面详细建模以及宇宙重子热力学状态研究。
...
TL;DR: 该研究在平坦Λ(t)CDM宇宙学框架下，分析了两种与哈勃膨胀率和暗能量密度成比例的相互作用暗能量模型，使用多种观测数据约束了模型参数，并计算了聚类参数S₈。


<details>
  <summary>Details</summary>
Motivation: 研究相互作用暗能量模型是为了探索暗能量与暗物质之间可能存在的相互作用，这有助于解决当前宇宙学观测中的一些张力问题，特别是关于结构形成参数S₈的测量值。

Method: 在背景和线性扰动两个层面进行分析，考虑两种相互作用形式：Q₁=εaHρ̄Λ(t)和Q₂=εHρ̄Λ(t)。使用联合数据集包括CMB距离先验、BAO、Pantheon+超新星、RSD和宇宙计时器H(z)数据，通过物质扰动的线性增长估计S₈参数。

Result: 从联合数据集得到约束：模型I的S₈=0.870±0.026，模型II的S₈=0.872±0.026。这些结果与标准ΛCDM模型的结果基本一致，表明相互作用暗能量模型在当前观测精度下与标准模型兼容。

Conclusion: 研究表明，在平坦Λ(t)CDM框架下，考虑与哈勃膨胀率和暗能量密度成比例的相互作用暗能量模型，得到的S₈参数约束与标准模型一致。这为理解暗能量与暗物质之间的可能相互作用提供了观测约束，但需要更高精度的数据来区分这些模型与标准ΛCDM模型。
...
TL;DR: 黑洞在超轻暗物质孤子中轨道衰减的预期被"打水漂"现象改变，孤子的偶极激发导致轨道半径准周期性变化，显著影响双黑洞并合时标。
...
Method: 将"打水漂"现象建模为受迫阻尼谐振子中的共振，通过孤子的偶极激发来解释轨道半径的准周期性变化，分析孤子相干响应如何改变轨道内物体的动力学。

Result: 孤子的偶极扰动可以显著修改黑洞的轨道动力学，当黑洞质量远小于孤子质量时，这种效应会改变并合时标，对超大质量黑洞演化、最后秒差距问题的解决和ULDM宇宙学中的引力波观测有重要影响。

Conclusion: 超轻暗物质孤子的偶极激发导致的"打水漂"现象是黑洞轨道动力学的重要机制，这一发现挑战了传统动力学摩擦的预期，对理解暗物质环境中黑洞并合过程具有重要意义。
...
TL;DR: 该研究测试了动态暗能量w₀wₐCDM（CPL）模型相对于ΛCDM模型的表现，使用CMB各向异性、透镜效应及晚期距离探针数据。发现CPL模型的推断结果强烈依赖于数据集选择，不同数据组合导致对当前宇宙加速状态的矛盾结论，表明对CPL模型的支持及其缓解哈勃张力的能力并非普遍存在。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估动态暗能量CPL框架在缓解哈勃张力和解释宇宙加速膨胀方面的有效性。由于不同观测数据集可能给出矛盾的暗能量约束，需要系统研究CPL模型在不同数据组合下的表现，以确定其支持是否具有普遍性，并识别可能的数据系统误差。

Method: 采用贝叶斯推断方法，将w₀wₐCDM（CPL）模型与ΛCDM模型进行比较。使用多种数据集组合：CMB各向异性和透镜数据；DESI DR2 BAO数据；SDSS-IV BAO共识汇编；横向/角度BAO汇编（BAOtr）；以及造父变星校准的PantheonPlus超新星Ia型数据（PP&SH0ES）。分析不同数据组合下的参数约束、贝叶斯证据和哈勃张力缓解程度。

Result: 结果显示出强烈的数据集依赖性：仅CMB数据时存在几何简并性，允许超加速（q₀<-1）；CMB+DESI DR2 BAO倾向于弱加速或近海岸膨胀（q₀≃0）；CMB+PP&SH0ES+BAOtr则给出常规适度加速（-1<q₀≲0）并显著缓解哈勃张力。所有组合中w(z→∞)=w₀+wₐ<-1，而复合期后红移处膨胀保持物质主导（q→1/2）。贝叶斯证据仅在包含PP&SH0ES和/或BAOtr时强烈支持CPL，其他组合则不确定或适度支持ΛCDM。

Conclusion: 结论表明对CPL模型的支持及其缓解哈勃张力的能力并非普遍存在，而是敏感依赖于所采用的晚期距离数据。这要求要么采用更灵活的晚期宇宙模型，要么更仔细地审视当前BAO测量中可能存在的残余系统误差。r_d基本保持不变表明H_0的变化源于晚期膨胀自由度而非早期宇宙物理。
...
TL;DR: 使用模拟推断框架对XXL巡天的X射线星系团样本进行宇宙学分析，结合X射线和弱引力透镜数据约束宇宙学参数和标度关系
...
Method: 采用基于模拟的推断框架，使用DELFI算法构建包含12个参数的前向模型，结合XXL选择函数和星系团群体建模，考虑系统误差如团中心偏移、光红移偏差和质量依赖的弱透镜偏差

Result: 得到宇宙学参数约束 $S_8 = 0.867 \pm 0.063$，与Planck和近期星系团测量一致；温度-质量关系与自相似预期一致，光度-温度关系斜率比自相似预测更陡；为所有XXL星系团提供了透镜校准的质量估计

Conclusion: 该分析为XXL样本提供了自洽的质量校准，支持未来的多探针宇宙学分析，展示了基于模拟的推断框架在联合约束宇宙学参数和天体物理标度关系方面的有效性
...
TL;DR: 该研究使用基于模拟的推理方法分析宇宙微波背景低多极矩的EB相关谱，以区分轴子暗能量和暗物质对宇宙双折射现象的解释。


<details>
  <summary>Details</summary>
Motivation: 宇宙双折射现象（CMB偏振面旋转）的物理起源尚不明确，可能是轴子暗能量或暗物质引起的。低多极矩的EB相关谱是区分这两种解释的关键，但其似然函数形式未知且高度非高斯。

Method: 采用基于模拟的推理技术，包括神经似然估计来推断EB相关的似然函数，以及神经后验估计来约束标量场质量、宇宙双折射振幅和仪器校准误差角。

Result: 发现标量场质量后验分布存在两个区域，以10^{-32} eV为分界点，反映了对宇宙双折射尺度依赖性的强敏感性。仪器校准误差和透镜B模贡献限制了完全排除暗能量场景的能力。

Conclusion: 基于模拟的推理方法能有效处理高度非高斯的EB相关似然函数，为区分宇宙双折射的物理起源提供了有力工具，但实际观测中的系统误差限制了区分能力。
...
TL;DR: 联合弱引力透镜与快速射电暴色散测量，通过多探针分析校准重子物理效应，显著改善宇宙学参数约束，特别是S8参数
...
Method: 采用联合分析方法，结合第四阶段弱引力透镜巡天数据与快速射电暴的色散测量。评估该方法同时约束宇宙学参数和重子反馈天体物理过程的能力，量化关键FRB系统误差（包括红移不确定性和源聚类）的影响。进一步扩展框架，纳入发光红星系和发射线星系的星系聚类测量，进行统一的6×2点分析。

Result: 即使考虑了系统误差，WL和FRBs的3×2点分析显著改善了宇宙学约束，与单独WL相比，S8的退化因子减少了约80%。FRBs单独只能约束关键重子参数log₁₀Mc和ηb的简并组合，而WL测量的加入打破了这种简并性。虽然WL、FRBs和星系聚类的联合分析收紧了对Ωm和log₁₀Mc的约束，但在S8约束方面并未显著超越WL和FRBs单独的结果。

Conclusion: 弱引力透镜与快速射电暴色散测量的联合分析是校准重子物理效应、改善宇宙学参数约束的有效方法，特别是在S8参数方面。多探针方法能够打破参数简并，但进一步加入星系聚类测量对S8约束的改善有限。
...
TL;DR: 该论文研究了暴胀期间额外场与暴胀子直接耦合产生的新型振荡性非高斯性信号，及其在大尺度结构中的观测特征和当前/未来星系巡天的探测前景。


<details>
  <summary>Details</summary>
Motivation: 暴胀期间额外场产生的原初非高斯性为探测超高能物理提供了重要窗口，但传统重场产生的振荡信号通常被抑制，标准等边非高斯性成为主要探测通道。本文旨在探索暴胀子与额外场直接耦合产生的新型振荡性非高斯性信号，这些信号同时具有轻重场的特征，可能开辟新的可观测参数空间。

Method: 通过理论建模分析暴胀子与额外场直接耦合产生的非高斯性特征，重点关注其在晚期大尺度结构中的可观测效应，特别是星系功率谱的尺度依赖偏置中的振荡调制。研究涵盖这些模型中出现的完整参数空间，并评估当前和未来星系巡天（如BOSS DR12）对这些振荡信号的探测灵敏度。

Result: 发现直接耦合可以产生多种新颖的观测相关信号，这些信号在星系功率谱的尺度依赖偏置中表现为围绕非平凡幂律的振荡调制。虽然对BOSS DR12数据的分析未发现显著信号，但证明了振荡性非高斯性相比非振荡情形具有更高的探测灵敏度，并首次给出了这一扩展参数空间的约束。

Conclusion: 暴胀子与额外场的直接耦合产生的新型振荡性非高斯性为宇宙学观测开辟了新的探测窗口，其振荡特征使近期的星系巡天具有更高的探测灵敏度。尽管现有数据未发现信号，但该方法显著提升了探测能力，并为未来实验提供了重要的理论框架和观测目标。
...
TL;DR: 该综述评估了S8张力问题，基于2026年新的CMB联合分析基准，揭示了DES和KiDS测量结果之间的显著分歧，并探讨了系统误差来源。


<details>
  <summary>Details</summary>
Motivation: 早期宇宙CMB观测与晚期宇宙探测之间在S8参数上存在持续差异，需要评估这一张力问题并理解不同测量结果之间的分歧。

Method: 采用统一的"联合CMB"框架整合Planck、ACT DR6和SPT-3G数据；编译2019-2026年的测量结果；分析DES Year 6和KiDS Legacy结果的分歧；检验系统误差来源如光测红移校准、内禀排列模型和剪切测量流程。

Result: 联合CMB分析得到S8=0.836^{+0.012}_{-0.013}；DES Year 6结果显示2.4σ-2.7σ的显著张力，而KiDS Legacy结果在<1σ内统计一致；不同探测方法（星系团计数、星系-星系透镜、红移空间畸变）呈现异质性结果。

Conclusion: 观测到的差异很大程度上源于特定巡天的系统误差效应，但ΛCDM之外的新物理可能性不能被排除；S8张力问题需要更深入的系统误差分析和跨探测方法的一致性验证。
...
TL;DR: 该论文研究了由大振幅原初曲率扰动坍缩形成行星质量原初黑洞时产生的标量诱导引力波背景，探讨了未来月球激光测距和卫星激光测距数据对其探测的可能性，并推导了零探测情况下的约束条件。


<details>
  <summary>Details</summary>
Motivation: 研究行星质量原初黑洞形成过程中产生的μHz频段引力波背景，探索未来激光测距实验探测这种信号的可能性，并建立与现有微引力透镜观测的联系。

Method: 推导大振幅原初曲率扰动坍缩形成行星质量原初黑洞时产生的标量诱导引力波背景理论模型，计算其在μHz频段的特征；基于未来月球激光测距和卫星激光测距实验的灵敏度，推导零探测情况下的原初黑洞种群约束；考虑电弱相变对行星质量原初黑洞丰度的影响。

Result: 获得了未来激光测距实验对原初黑洞种群的投影约束，建立了这些约束与HSC合作组对仙女座星系微引力透镜观测结果之间的联系，为行星质量原初黑洞的探测提供了新的观测窗口。

Conclusion: 未来月球激光测距和卫星激光测距数据有望探测行星质量原初黑洞形成产生的μHz引力波背景，为零探测情况下的原初黑洞种群提供重要约束，并与现有微引力透镜观测形成互补。
...
<details>
  <summary>Details</summary>
Motivation: 理解再电离时期的演化和时间对于天体物理学和宇宙学至关重要，但直接映射电离气泡面临高红移、选择偏差和前景干扰等挑战

Method: 利用21厘米信号与线强度映射信号之间的实空间互相关函数ξ_{21,ν}，通过其随分离距离r的变化特征来重建气泡尺寸分布

Result: ξ_{21,ν}(r)在特定尺寸气泡开始形成时偏离饱和水平，并随再电离进程从正相关演变为负相关，在特定半径气泡达到峰值丰度时达到最大反相关

Conclusion: 该方法对天体物理模型和再电离时间/拓扑结构变化具有鲁棒性，通过捕捉气泡的局域性质补充了传统的傅里叶空间估计器，为驱动宇宙再电离的源提供了新见解
...
TL;DR: JWST观测显示早期宇宙再电离迅速完成，早期暗能量模型能同时解释高红移星系丰度和再电离历史，无需极端物理参数。


<details>
  <summary>Details</summary>
Motivation: JWST观测到z~11-6的Lyman-α发射线表明宇宙再电离在~700 Myr内迅速完成，需要大量LyC光子。标准ΛCDM框架下需要极高LyC逃逸分数(f_esc≳0.2)或超暗星系主导贡献。早期暗能量(EDE)模型能解释高红移星系丰度，本研究探讨EDE模型下的再电离历史。

Method: 在早期暗能量(EDE)范式下建模再电离历史。EDE模型最初为解决哈勃张力提出，也能解释JWST观测到的高红移星系丰度，无需极端恒星形成效率。模型计算所需的LyC逃逸分数和UV光度密度。

Result: EDE模型产生的再电离历史与当前约束一致，仅需中等LyC逃逸分数(f_esc~0.05-0.1)和UV光度密度(M_UV≲-17至-15)。相比标准ΛCDM框架，无需极端参数就能满足再电离光子预算。

Conclusion: 一旦关键天体物理参数得到更好约束，再电离历史可作为早期暗能量宇宙学的独立互补探针。EDE模型能同时解释高红移星系丰度和再电离观测，提供一致的宇宙学框架。
...
TL;DR: 利用Illustris TNG-100模拟分析低表面亮度星系(LSBGs)及其卫星系统的特性，发现LSBGs比高表面亮度星系(HSBGs)拥有更多卫星且速度弥散更大，卫星系统以旋转为主并与主盘呈反向旋转，表明LSBGs经历较少合并事件。


<details>
  <summary>Details</summary>
Motivation: 在ΛCDM宇宙学框架下，深入理解低表面亮度星系(LSBGs)的形成机制，通过分析其卫星系统的特性来揭示LSBGs与高表面亮度星系(HSBGs)在形成演化上的差异。

Method: 使用Illustris TNG-100宇宙学模拟，分析中央LSBGs及其卫星系统的不同特性，包括晕的自旋和浓度、卫星的空间和运动学分布、卫星数量与表面亮度的关系、卫星相对切向速度与表面亮度的关系等。

Result: 1. LSBGs晕的自旋和浓度与HSBGs存在差异；2. LSBGs比HSBGs拥有更多卫星且速度弥散更大；3. 卫星数量与表面亮度存在连续关系，特别是对于大质量中央星系；4. 表面亮度与卫星相对切向速度相关；5. 给定恒星质量下，LSBGs的存在与其以旋转为主的卫星系统强相关；6. LSBGs中卫星系统与主盘呈系统性反向旋转。

Conclusion: LSBGs的卫星系统特性表明这些星系没有经历过显著高频率的合并事件，这与径向轨道主导的高表面亮度星系系统形成对比，支持LSBGs通过相对平静的演化路径形成的观点。
...
TL;DR: 超新星反馈在紧凑星暴星系中可能通过清除中性气体通道促进电离光子逃逸，但同时也可能降低电离光子产生效率


<details>
  <summary>Details</summary>
Motivation: 研究超新星反馈如何影响紧凑星暴星系中电离光子的逃逸机制，这对于理解宇宙再电离时期星系的作用至关重要

Method: 将1242个蓝莓星系与瞬变源发现报告交叉匹配，识别出11个超新星宿主星系，并与更大的蓝莓星系群体进行比较分析

Result: 超新星宿主星系表现出更高的恒星形成率、更剧烈的近期恒星形成历史、更高的恒星质量、更蓝的UV斜率，但电离光子产生效率显著降低

Conclusion: 超新星驱动的反馈可能通过清除中性气体通道促进电离光子逃逸，但同时也可能通过抑制恒星形成而降低电离光子产生效率
...
TL;DR: CenA和M83星系对实际上正在相互靠近，类似于银河系与仙女座星系的运动，通过定时论证估算其总质量约为6.36×10¹² M☉


<details>
  <summary>Details</summary>
Motivation: 尽管CenA和M83的观测视向速度并不明显显示相互吸引，但本研究旨在证明它们实际上正在相互靠近，并估算该系统的总质量，将其作为研究本地群类似系统的近距离类比

Method: 使用定时论证方法，通过AbacusSummit模拟中的类似星系对进行校准，假设视线速度主要受向系统质心运动主导，从而估算CenA/M83系统的总质量

Result: 估算得到CenA/M83系统的总质量为(6.36±1.30)×10¹² M☉，该结果与基于维里质量测量和K波段光度-质量比的独立估计一致

Conclusion: CenA和M83确实在相互靠近，其一致的束缚特征和可靠的质量测定使该系统成为本地群的有力近距离类比，NGC 4945可能作为CenA的主要扰动源（类似大麦哲伦云）
...
<details>
  <summary>Details</summary>
Motivation: 高红移类星体对是超大质量黑洞合并的前身，但现有方法对视线距离的测量存在较大不确定性，限制了对其特性的研究。

Method: 通过合成类星体邻近区光谱，使用简单的峰值检测算法来区分不同视线距离场景，评估该方法在不同天空平面分离距离下的准确性。

Result: 对于小天空平面分离（10-100 pkpc），能轻松区分视线距离小于1 pMpc和大于1 pMpc的情况；当真实距离≥3 pMpc时，估计精度约0.2 pMpc；对于1 pMpc的大天空平面分离，仍能区分小于4 pMpc和大于4 pMpc的情况，精度约0.5 pMpc。
...
TL;DR: 半人马座A的卫星星系系统存在内区明亮卫星星系缺失的异常现象，研究表明这可能是由AGN反馈机制导致的。


<details>
  <summary>Details</summary>
Motivation: 半人马座A卫星星系系统呈现一个奇特现象：整体数量符合理论预期，但内区（d<150 kpc）明亮卫星星系数量明显不足。研究者试图解释这种空间分布异常背后的物理机制。

Method: 使用Galacticus半解析模型结合高分辨率N体模拟的合并树，研究各种可能的淬灭机制。基准模型基于银河系校准，探索了潮汐瓦解、再电离淬灭、以及周围介质吸积抑制等多种物理场景。

Result: 基准模型能再现半人马座A整体卫星星系数量，但高估了内区明亮卫星星系（Mv < -15.8）数量约4±1倍。这种缺失与半人马座A的千秒差距尺度射电瓣空间重合，表明存在强大的环境驱动因素。研究发现z<5时期的AGN驱动热反馈能显著抑制卫星星系中的恒星形成，有效截断内区光度函数的亮端。

Conclusion: 半人马座A的"奇特案例"可能为主星系内AGN反馈调控其矮星系卫星的生存和演化提供了证据。AGN反馈机制能显著影响卫星星系的恒星形成历史，解释内区明亮卫星星系的缺失现象。
...
TL;DR: 该研究发现，矮球状星系的动力学质量推断对恒星密度分布形状的假设高度敏感，标准动力学质量估计器可能存在高达10倍的系统误差。


<details>
  <summary>Details</summary>
Motivation: 传统上，矮球状星系的动力学质量推断依赖于对恒星密度分布形状的简化假设。然而，这些假设可能引入显著的系统误差，影响对暗物质晕性质的准确理解。

Method: 研究采用灵活的恒星密度模型拟合观测数据，放松标准假设，利用维里定理将恒星密度分布形状的不确定性传播到动力学质量推断中，考虑不同暗物质晕密度分布和空间范围。

Result: 研究发现，银河系矮球状星系的观测结构多样性导致标准动力学质量估计器存在高达10倍的系统误差范围。考虑这些不确定性会显著改变动力学标度关系（如包围质量-恒星质量关系、径向加速度关系）的表现。

Conclusion: 矮球状星系动力学质量推断对恒星密度分布形状的假设高度敏感，必须考虑观测和系统不确定性才能获得可靠的暗物质晕性质估计，这对理解矮星系形成和暗物质性质具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 以往MgII晕的研究仅限于单个天体分析，限制了对外流特性的统计理解。需要通过对更大样本的统计研究，获得星系尺度外流特性的群体层面见解

Method: 使用深空MUSE观测47个0.7<z<2.0的恒星形成星系，将先前开发的单个MgII晕外流建模框架扩展到更大样本，将外流建模为径向加速壳层集合

Result: 检测到延伸至数十千秒差距的MgII发射；外流从约60 km/s的发射速度线性加速，最大速度与恒星质量相关达~490 km/s；内区高度不透明；晕尺寸分布峰值在~5 kpc，延伸至~20 kpc；紧凑晕尺寸与恒星质量相关，延伸晕则不相关

Conclusion: 这是首次对MgII发射晕的统计研究，证实了恒星形成驱动外流的存在，揭示了外流特性与星系性质的相关性，紧凑晕与延伸晕可能具有不同的驱动机制
...
TL;DR: ALMA Band1观测揭示了rhoOphW光解离区在36-44GHz的精细结构，其微波超辐射谱呈现幂律衰减（α=-0.78±0.05），无精细结构特征，且高频空间频率下辐射功率相对IR信号增强


<details>
  <summary>Details</summary>
Motivation: rhoOphW是明亮的微波超辐射源，其丝状结构与IR PAH发射相关。需要深度高保真成像和光谱观测来揭示EME信号的详细形态（避免成像先验），同时搜索光谱精细结构并约束高频衰减的谱指数

Method: 使用ALMA Band1（36-44GHz）马赛克观测获得深度去卷积的丝状结构图像，将其作为模板在uv平面通过互相关提取光谱。通过模拟和近红外辅助数据的互相关来估计通量损失和偏差

Result: 光谱呈现幂律分布，无检测到的精细结构，谱指数α=-0.78±0.05（频率空间），沿丝状结构有变化。Band1在高空间频率下相对IR信号的功率增强（在~20"处比~100"处高两倍）。发现一个紧凑的EME源（无IR对应体），嵌入在强丝状Band1信号中，而IRAC图在该区域平滑

Conclusion: 研究提供了多频强度估计用于光谱建模，揭示了rhoOphW微波超辐射的幂律衰减特性和空间分布特征，支持旋转尘埃颗粒电偶极辐射的解释，同时发现了无IR对应体的紧凑EME源，表明可能存在不同的发射机制或环境条件
...
TL;DR: 利用JWST中波段测光数据，研究宇宙正午时期（z=1.2-2.4）恒星形成星系的星云消光曲线和尘埃几何结构，发现质量更大的星系尘埃更多，且浅的Reddy星云消光曲线比传统Cardelli曲线更符合观测，表明尘埃分布呈斑块状、非均匀结构。


<details>
  <summary>Details</summary>
Motivation: 研究宇宙正午时期（z=1.2-2.4）恒星形成星系的星云消光曲线和尘埃几何结构，以理解高红移星系的尘埃特性和分布模式。传统上常使用Cardelli等（1989）的消光曲线，但需要验证其在早期宇宙环境中的适用性。

Method: 使用JWST/MegaScience/UNCOVER的中波段测光数据，测量209个恒星形成星系的Hα发射线流量，其中66个星系还测量了Paβ流量。通过Paβ/Hα线比分析尘埃消光，并与MOSDEF巡天的光谱测量结果（Hα/Hβ）进行比较，评估不同消光曲线模型。

Result: 1. Paβ/Hα线比随恒星质量和恒星形成率增加而显著增加，表明质量更大的星系尘埃更多；2. 浅的Reddy等（2025）星云消光曲线比传统Cardelli曲线更符合观测，尤其对高质量星系；3. 星云消光与轴比无相关性，表明尘埃可能局限于小尺度团块；4. 使用浅消光曲线可使高质量星系的A_Hα,neb增加达1星等。

Conclusion: 宇宙正午时期恒星形成星系的尘埃几何结构呈斑块状和非均匀分布，尤其在高质量星系中。浅的Reddy星云消光曲线比传统Cardelli曲线更符合观测，表明需要重新考虑高红移星系的尘埃模型。JWST中波段测光为大规模样本的发射线研究和尘埃特性统计约束提供了强大能力。
...
TL;DR: 利用ALMA-QUARKS巡天数据在121个大质量恒星形成团块中发现323条超细尘埃纤维结构，宽度约0.01 pc，比Herschel发现的附近云中尘埃丝窄约10倍，密度更高（N_H2 = 10^23-10^24 cm^-2），质量-长度关系符合M∝L^2。


<details>
  <summary>Details</summary>
Motivation: 探索大质量恒星形成团块中更小尺度的丝状结构，以理解恒星形成过程中多尺度丝状结构的作用。先前Herschel等观测发现了约0.1 pc宽的丝状结构，但更高分辨率的ALMA数据可能揭示更精细的结构。

Method: 使用ALMA-QUARKS巡天的1.3 mm连续谱发射数据（线分辨率约900 AU，对应距离约3 kpc），通过FilFinder软件识别丝状结构，再用RadFil软件测量其宽度等物理参数。样本包含121个位于不同银河系环境（银心距约0.5-12.7 kpc）的大质量恒星形成团块。

Result: 在121个团块中发现超过323条超细纤维（fibrils），典型宽度约0.01 pc，比Herschel发现的附近云中尘埃丝窄约10倍。质量-长度关系符合M∝L^2，与先前银河系丝状结构一致，但密度显著更高（N_H2 = 10^23-10^24 cm^-2 vs Herschel的10^20-10^23 cm^-2）。

Conclusion: 这项工作提供了大质量团块中超细纤维的大样本，继先前在附近分子云中发现0.1 pc宽丝状结构和内部速度相干纤维后，进一步强调了丝状结构在恒星形成中跨多个物理尺度的关键作用。
...
TL;DR: 研究通过多波段数据（X射线、中红外、射电）分析低红移（z<0.2）后星暴星系、恒星形成星系和宁静星系中的AGN活动，发现后星暴星系的AGN辐射并未显著增强，AGN在低红移星系淬灭中的作用可能更偏向"预防性"而非"喷出性"反馈。


<details>
  <summary>Details</summary>
Motivation: 虽然AGN反馈被认为是星系演化中恒星形成淬灭的关键驱动机制，但在附近淬灭星系中缺乏强AGN的现象引发了对其有效性的质疑。本研究旨在通过分析后星暴星系（PSBs）——处于淬灭过渡阶段的星系——中的AGN活动，探讨AGN在低红移星系淬灭过程中的实际作用。

Method: 使用eROSITA/eFEDS（X射线）、WISE（中红外）和FIRST（射电）的多波段数据，分析z<0.2的后星暴星系、恒星形成星系和宁静星系中的AGN活动。评估不同演化阶段的AGN发生率和强度，并对未探测到的星系应用叠加技术以恢复平均AGN性质。通过比较观测光度与恒星形成预期光度（L_obs/L_SF）来评估AGN贡献。

Result: 后星暴星系的射电和X射线辐射主要受恒星形成主导，而非AGN活动。虽然后星暴星系的中红外AGN发生率是恒星形成星系的两倍，但其估计的AGN光度远低于文献中典型的中红外AGN。总体上，后星暴星系相对于质量和红移匹配的恒星形成星系和宁静星系，并未显示出显著增强的AGN辐射。可能存在被遮蔽的低光度AGN，但其作用有限。

Conclusion: 在低红移星系中，AGN在淬灭过程中的作用比剧烈移除气体更为微妙——反馈机制更可能是"预防性"的而非"喷出性"的。后星暴星系中可能存在的低光度AGN可能由前身星暴残留的气体提供燃料，但不太可能在淬灭过程中起主导作用。
...
TL;DR: 利用HST WFC3/UVIS的F225W波段扩展哈勃前沿场观测，测量0.4<z<0.7星系紫外光度函数，探测到低光度星系，首次在z~0.55将紫外光度函数测量延伸至M_UV<-13.5星等。


<details>
  <summary>Details</summary>
Motivation: 通过扩展哈勃前沿场观测到F225W波段，结合强引力透镜放大效应，探测0.4<z<0.7的低光度星系，填补紫外光度函数在低光度端的测量空白，研究光度函数在暗端的可能转折。

Method: 使用HST Cycle-27 GO-15940的F225W观测数据，结合深度近紫外观测和前景星系团的强引力透镜放大效应，通过混合测光-光谱红移选择识别152个暗星系，采用最大似然估计拟合Schechter函数参数，并引入曲率参数δ考虑光度函数在暗端的可能转折。

Result: 在0.4<z<0.7识别152个暗星系(-19.5<M_UV<-12.1)，在z~0.55将紫外光度函数测量延伸至M_UV<-13.5星等，得到暗端斜率α=-1.324^{+0.072}_{-0.074}，以3σ置信度排除M_UV=-15.5星等以上的光度函数转折。

Conclusion: 成功将紫外光度函数测量延伸至前所未有的低光度范围，未发现M_UV=-15.5星等以上的光度函数转折，为星系形成和演化模型提供了重要的观测约束。
...
### [26] [Characterising Ly$α$ damping wings at the onset of reionisation: Evidence for highly efficient star formation driven by dense, neutral gas in UV-bright galaxies at $z>9$](https://arxiv.org/abs/2602.11783)
...
TL;DR: JWST观测发现z>9的高红移星系存在过量中性氢气体，驱动了远超本地宇宙标准的高效恒星形成，这解释了宇宙黎明时期UV明亮星系的成因。


<details>
  <summary>Details</summary>
Motivation: 解决当代河外天体物理学中的一个主要难题：为什么在红移z≳9处存在明显过量的UV明亮星系。通过研究这些早期星系的特性，理解宇宙再电离开始时星系晕中中性氢气体的聚集过程。

Method: 使用JWST/NIRSpec棱镜光谱仪观测红移z>9的星系，获得足够的信噪比来仔细建模其静止框架下的UV到光学连续谱和发射线。特别关注阻尼莱曼α吸收特征，推导中性氢柱密度和恒星形成率面密度。

Result: 所有星系都表现出极高的恒星形成效率，消耗时间尺度为10-100 Myr，远超本地宇宙的Kennicutt-Schmidt关系和最先进的星系形成模拟预测。致密的中性氢气体也导致这些星系偏离基本金属丰度关系，但尘埃-气体比与本地星系一致（最低金属丰度视线除外）。

Conclusion: 这些结果为原始中性氢气体对早期星系组装的影晌提供了首个可靠的观测约束，表明高效恒星形成和低尘埃消光的结合很可能解释了宇宙黎明时期星系的UV亮度。
...
### [27] [TAVAE: A VAE with Adaptable Priors Explains Contextual Modulation in the Visual Cortex](https://arxiv.org/abs/2602.11956)
*Balázs Meszéna,Keith T. Murray,Julien Corbo,O. Batuhan Erkat,Márton A. Hajnal,Pierre-Olivier Polack,Gergő Orbán*

Main category: q-bio.NC

TL;DR: 研究证实视觉系统能够按需学习灵活的任务特异性上下文先验，并在视觉皮层入口级部署这些先验。


<details>
  <summary>Details</summary>
Motivation: 虽然证据表明适应会导致反映自然图像结构的先验，但尚不清楚在执行特定任务学习时是否能类似地灵活获取先验。本研究旨在探究视觉系统是否能够按需学习任务特异性先验。

Method: 构建了针对简单辨别任务优化的V1生成模型，并与执行类似任务的小鼠大规模记录数据结合分析。扩展了VAE形式，使任务能够通过重用先前学习到的表征来高效获取。使用任务摊销VAE学习到的任务特异性先验来研究当呈现违反训练任务统计的刺激时，小鼠和模型中的偏差。

Result: 学习到的任务统计与传入感官证据之间的不匹配在TAVAE后验中产生了刺激类别不确定性的特征，反映了V1记录中双峰响应曲线的特性。任务优化的生成模型解释了V1群体活动的关键特征，包括群体响应的日内更新。

Conclusion: 研究结果证实视觉系统能够按需学习灵活的任务特异性上下文先验，并在视觉皮层入口级（V1）部署这些先验。
...
### [28] [Methodological Variation in Studying Staff and Student Perceptions of AI](https://arxiv.org/abs/2602.11158)
*Juliana Gerard,Morgan Macleod,Kelly Norwood,Aisling Reid,Muskaan Singh*

Main category: cs.HC

TL;DR: 比较评估学生和教职员工对AI感知的不同方法学方法，发现单一数据源的不同分析方法会产生不同结果


<details>
  <summary>Details</summary>
Motivation: 现有研究通常使用单一量化或质化方法，或混合方法比较不同数据源来评估AI感知，但缺乏对同一数据源不同分析方法差异的系统比较

Method: 收集两种质化数据：独立评论和结构化焦点小组；对每种数据源进行两种分析：情感/立场分析（测量整体积极性/消极性）和内容分析（评论的词云和焦点小组的主题分析）

Result: 不同分析方法对同一数据源会产生不同结果；整体积极性/消极性测量与详细内容分析呈现不同图景；主题分析揭示了学生和教职员工之间的相似性和差异性

Conclusion: 方法学选择显著影响研究结果，这对机构背景下的决策和先前研究的比较具有重要启示；需要更谨慎地解释不同方法得出的结论
...
### [29] [BIRD: A Museum Open Dataset Combining Behavior Patterns and Identity Types to Better Model Visitors' Experience](https://arxiv.org/abs/2602.11160)
...
<details>
  <summary>Details</summary>
Motivation: 文化遗产领域缺乏开放数据集，现有数据通常是针对特定模型评估的临时数据，无法全面建模参观者体验

Method: 2019年2-3月对51名参与者进行眼动追踪研究，让他们自由探索3层博物馆的400多件艺术品，收集上下文数据、行为数据和反馈数据

Result: 构建了包含人口统计、偏好、访问习惯、动机、社交背景、时空轨迹、注视数据和满意度等信息的综合数据集，能够重现文献中的参观者身份特征和Veron-Levasseur档案

Conclusion: 该数据集将用于通过个性化兴趣点数量、停留时间和信息量来改进博物馆推荐路径的质量
...
### [30] [DiSCoKit: An Open-Source Toolkit for Deploying Live LLM Experiences in Survey Research](https://arxiv.org/abs/2602.11230)
...
Result: 成功创建了能够处理聊天数据记录、AI行为操控等功能的工具包，支持在Qualtrics等平台上部署实时LLM交互实验。

Conclusion: DiSCoKit解决了人机交互研究中AI刺激部署的技术障碍，为社会科学研究提供了实用的实验工具，同时指出了部署的可能性和局限性。
...
### [31] [Same Feedback, Different Source: How AI vs. Human Feedback Shapes Learner Engagement](https://arxiv.org/abs/2602.11311)
*Caitlin Morris,Pattie Maes*

Main category: cs.HC

TL;DR: 研究探讨了反馈来源（AI vs 人类助教）对学习者参与度和评价的影响，发现即使反馈内容完全相同，学习者对人类助教来源的反馈投入更多时间和努力，且评价机制不同。


<details>
  <summary>Details</summary>
Motivation: 随着AI与人类教师共同参与教育，理解学习者对反馈来源的归因效应对于设计有效的混合AI-人类教育系统至关重要。需要探究学习者如何根据反馈来源（AI或人类）来调整其参与行为和评价方式。

Method: 设计了创意编程界面，在控制反馈内容完全相同的情况下，将参与者分为两组：一组被告知反馈来自AI，另一组被告知来自人类助教。所有反馈实际上均由LLM生成。测量了学习者的参与度（时间和努力程度）以及对反馈的评价。

Result: 1. 反馈来源显著影响参与度：人类助教条件下的学习者投入更多时间和努力（效应量d=0.88-1.56）。2. 评价机制不同：AI反馈的评价与对AI的先前信任度高度相关（r=0.85），而人类助教反馈的评价则与感知真诚度相关（r=0.65）。

Conclusion: 反馈来源不仅影响学习者的参与行为，还影响其评价机制。这对混合教育系统设计有重要启示：需要考虑如何呈现反馈来源以优化学习效果，并理解不同来源反馈的评价标准差异。
...
### [32] [Situated, Dynamic, and Subjective: Envisioning the Design of Theory-of-Mind-Enabled Everyday AI with Industry Practitioners](https://arxiv.org/abs/2602.11342)
*Qiaosi Wang,Jini Kim,Avanita Sharma,Alicia,Lee,Jodi Forlizzi,Hong Shen*

Main category: cs.HC

TL;DR: 研究通过13次协同设计会议探讨了心理理论（ToM）能力在日常AI产品设计中的应用，提出了三项设计建议并揭示了设计张力


<details>
  <summary>Details</summary>
Motivation: 虽然心理理论（ToM）被认为是人类社交互动的核心能力，AI领域的ToM能力研究也在增加，但如何将这种能力转化为日常用户AI产品的设计和体验尚不清楚。研究旨在探索ToM能力如何应用于实际AI产品设计，弥合理论研究与实际应用之间的差距。

Method: 研究采用协同设计方法，与26名美国AI从业者进行了13次协同设计会议。通过让从业者共同设想、反思和提炼设计建议，探索ToM能力在日常AI产品和服务中的应用，既展望未来又扎根于AI设计和开发实践的现实。

Result: 分析揭示了三个相互关联的设计建议：1）ToM-enabled AI应置于塑造用户心理状态的社会背景中；2）应响应心理状态的动态特性；3）应适应主观个体差异。研究在每个建议中都发现了设计张力，揭示了从业者设想的ToM-enabled AI未来与当前AI设计开发实践现实之间的差距。

Conclusion: 研究指出需要超越静态、推理驱动的ToM方法，转向将ToM设计为支持持续人机交互循环的普遍能力。这些发现为未来ToM-enabled AI产品的设计提供了实践指导，强调了将ToM能力整合到动态、情境化的人机交互中的重要性。
...
### [33] [Interpretive Cultures: Resonance, randomness, and negotiated meaning for AI-assisted tarot divination](https://arxiv.org/abs/2602.11367)
*Matthew Prock,Ziv Epstein,Hope Schroeder,Amy Smith,Cassandra Lee,Vana Goblot,Farnaz Jahanbakhsh*

Main category: cs.HC

TL;DR: 研究AI在塔罗牌占卜等主观解释性实践中的应用，探讨用户如何寻求、协商和评估"共鸣"解释，并提出支持解释性意义建构的AI设计建议


<details>
  <summary>Details</summary>
Motivation: 生成式AI工具在创意和分析任务中日益普及，但在主观、多元、非因果的解释性实践中的作用仍不清楚。本研究旨在理解AI在塔罗牌占卜这类占卜实践中的应用，其中意义是主观的、多元的且非因果的

Method: 基于对塔罗牌实践者的访谈，结合Hartmut Rosa的共鸣理论，研究用户在查询与解释数据之间不存在因果关系的情境下，如何寻求、协商和评估共鸣解释

Result: 识别出实践者将AI融入解释工作流程的多种方式：用于应对不确定性和自我怀疑、探索替代视角、简化或扩展现有占卜实践。发现用户寻求的是与个人经历产生共鸣的解释而非"正确"答案

Conclusion: 提出支持解释性意义建构的AI系统设计建议，要求系统在不消除模糊性或限制用户能动性的前提下，支持主观解释过程，保持开放性而非提供确定性答案
...
### [34] [Data-driven modelling of low-dimensional dynamical structures underlying complex full-body human movement](https://arxiv.org/abs/2602.11492)
...
Method: 采用神经常微分方程（NODEs）建模非周期性全身运动的时间演化，以棒球投球作为代表性目标运动，构建低维潜在动力学系统。

Result: 验证实验表明，NODE动力学模型能准确预测复杂投球运动的时间演化（R² > 0.45），仅使用前约8%的时间序列数据就能解释投球动作后半段约50%的方差。

Conclusion: 研究证实了将动力学系统方法扩展到更复杂、生态效度更高的人类运动的潜力，表明后续运动可根据潜在空间中ODE定义的动力学从初始条件演化而来。
...
### [35] [An Educational Human Machine Interface Providing Request-to-Intervene Trigger and Reason Explanation for Enhancing the Driver's Comprehension of ADS's System Limitations](https://arxiv.org/abs/2602.11507)
*Ryuji Matsuo,Hailong Liu,Toshihiro Hiraoka,Takahiro Wada*

Main category: cs.HC

TL;DR: 提出基于语音的教育性人机界面，通过提供接管请求触发线索和原因，帮助驾驶员理解自动驾驶系统的局限性，提高接管安全性和有效性。


<details>
  <summary>Details</summary>
Motivation: L3级自动驾驶系统在超出运行设计域时会发出接管请求，但复杂交通场景中驾驶员可能同时感知到多个潜在触发因素，导致接管时犹豫或困惑。驾驶员需要清晰理解系统局限性才能确保安全接管。

Method: 提出基于语音的教育性人机界面，在接管请求中提供触发线索和原因说明。通过驾驶模拟器进行组间实验，比较不同HMI条件对驾驶员理解系统局限性的影响。

Result: 实验结果显示，在接管请求中加入有效的触发线索和原因能改善驾驶员对系统局限性的理解。使用该方法的参与者能更主动地在接管请求失败时接管控制，且碰撞次数低于其他HMI条件。

Conclusion: 通过提出的方法持续增强驾驶员对自动驾驶系统局限性的理解，有助于实现更安全有效的实时交互，提高L3级自动驾驶系统的接管安全性。
...
### [36] [Implications of AI Involvement for Trust in Expert Advisory Workflows Under Epistemic Dependence](https://arxiv.org/abs/2602.11522)
*Dennis Kim,Roya Daneshi,Bruce Draper,Sarath Sreedharan*

Main category: cs.HC

TL;DR: 研究探讨AI助手如何影响用户对人类专家、AI系统及人机协作团队的信任，通过课程规划任务实验发现专家使用AI的方式会影响用户对其专业能力和信任度的感知。


<details>
  <summary>Details</summary>
Motivation: 随着AI工具在医疗、法律、金融等专业领域的应用日益增多，需要理解AI参与如何影响用户对人类专家、AI系统及其组合的信任，这对人机混合团队的设计至关重要。

Method: 采用用户研究（N=77），设计模拟课程规划任务，比较不同条件：AI的存在与否以及人机协作的具体模式，分析用户对专家专业能力和信任度的感知。

Result: 虽然顾问制定正确课程表的能力很重要，但用户对专业能力和信任的感知也受到专家如何使用AI助手的影响。专家使用AI的方式会影响用户对其专业性的判断。

Conclusion: 在设计人机混合团队时，需要考虑专家使用AI的方式如何影响最终用户对推荐者专业能力的感知，这对依赖用户采纳建议的应用场景尤为重要。
...
### [37] [Behavioral Indicators of Overreliance During Interaction with Conversational Language Models](https://arxiv.org/abs/2602.11567)
*Chang Liu,Qinyi Zhou,Xinjie Shen,Xingyu Bruce Liu,Tongshuang Wu,Xiang 'Anthony' Chen*

Main category: cs.HC

TL;DR: 该研究通过分析用户与LLM交互的行为模式，识别出与过度依赖相关的五种行为特征，为检测和缓解AI过度依赖提供设计启示。


<details>
  <summary>Details</summary>
Motivation: LLM在广泛场景中的使用带来了过度依赖风险，其幻觉可能隐藏错误信息。现有方法难以检测动态复杂情境中的过度依赖，需要从用户行为模式角度进行研究。

Method: 收集77名参与者在三个真实任务中与注入可信错误信息的LLM的交互日志，通过语义编码和聚类分析用户交互片段，识别与过度依赖相关的行为模式。
...
### [38] ["I Was Told to Come Back and Share This": Social Media-Based Near-Death Experience Disclosures as Expressions of Spiritual Beliefs](https://arxiv.org/abs/2602.11663)
*Yifan Zhao,Yuxin Fang,Yihuan Chen,RAY LC*

Main category: cs.HC

TL;DR: 分析200个TikTok视频发现，濒死体验叙事常被用于表达个人意义，多数包含精神宗教主题，这些内容能引发更多互动和深度信仰对话


<details>
  <summary>Details</summary>
Motivation: 虽然学者研究过个人如何分享濒死体验，但缺乏对当今社交媒体平台上这些叙事如何被协作传播的研究，需要探索在线平台如何促进社区层面的精神参与

Method: 随机抽样分析200个带有#nde及相关标签的TikTok视频，采用内容分析方法，同时分析评论区的互动模式

Result: 濒死体验叙事常被用于表达个人意义，多数帖子包含精神宗教主题；包含精神主题的视频能吸引更多互动，促进关于信仰和意义的深度对话

Conclusion: 在线平台能促进社区层面的精神参与，研究结果为设计支持专业社区共享表达和连接的空间提供了启示
...
### [39] [Mapping the Landscape of Affective Extended Reality: A Scoping Review of Biodata-Driven Systems for Understanding and Sharing Emotions](https://arxiv.org/abs/2602.11710)
*Zhidian Lin,Allison Jing,Ziyuan Qu,Fabio Zambetta,Ryan M. Kelly*

Main category: cs.HC

TL;DR: 对82篇论文进行范围综述，提出"情感扩展现实"概念，分析XR系统中使用生物数据理解情感的技术、交互技术和评估方法，绘制当前情感XR研究版图。


<details>
  <summary>Details</summary>
Motivation: HCI文献中包含许多使用生物数据理解情感的XR系统，但这些系统尚未被系统性地整合和映射。需要建立一个连贯的研究框架来理解情感XR系统的现状、技术和设计挑战。

Method: 对82篇探索生物数据、情感和XR交叉领域的论文进行范围综述，分析这些系统使用的技术、交互技术以及评估其有效性的方法。

Result: 绘制了当前情感XR研究版图，揭示了情感共享目标的多样性；展示了HCI研究者如何探索XR生物反馈系统的交互流程设计，突出了关键设计维度和理解情感的挑战；识别了未充分利用的情感共享方法。

Conclusion: 提出了"情感扩展现实"概念框架，系统性地映射了该领域的研究现状，为未来情感XR研究提供了设计维度和研究方向，指出了情感共享的新机会。
...
### [40] [Building Intelligent User Interfaces for Human-AI Alignment](https://arxiv.org/abs/2602.11753)
*Danqing Shi*

Main category: cs.HC

TL;DR: 提出一个用于分析用户界面如何改善人-AI对齐的参考模型框架


<details>
  <summary>Details</summary>
Motivation: 当前AI对齐研究主要关注训练算法，而用户界面常被忽视或仅视为实现细节，但实际上用户界面是对齐的关键因素

Method: 引入一个参考模型，提供系统化框架分析用户界面在何处以及如何改善人-AI对齐；通过两个案例研究和包含六个用户界面的初步调查来展示结构化分类法

Result: 建立了用户界面在人-AI对齐中的系统化分析框架，展示了通过人机交互推进对齐的机会

Conclusion: 用户界面不应仅被视为实现细节，而是改善人-AI对齐的关键因素；该参考模型为系统化分析界面贡献提供了框架，为人机交互领域推进对齐研究开辟了新方向
...
### [41] [V-SHiNE: A Virtual Smart Home Framework for Explainability Evaluation](https://arxiv.org/abs/2602.11775)
*Mersedeh Sadeghi,Simon Scholz,Max Unterbusch,Andreas Vogelsang*

Main category: cs.HC

TL;DR: V-SHiNE是一个基于浏览器的智能家居仿真框架，用于可扩展和真实地评估解释质量，通过模拟环境和行为来研究解释对用户信任和解释的影响


<details>
  <summary>Details</summary>
Motivation: 在自主智能家居系统中，解释对于帮助用户理解和信任决策至关重要，但评估解释质量和影响在该领域仍然存在方法论上的困难

Method: 开发了一个基于浏览器的智能家居仿真框架V-SHiNE，允许研究人员配置环境、模拟行为、插入自定义解释引擎，并提供灵活的交付模式和丰富的交互日志记录

Result: 通过159名参与者的研究证明了该框架的可行性，V-SHiNE提供了一个轻量级、可复现的平台，用于推进以用户为中心的可解释智能系统评估

Conclusion: V-SHiNE填补了智能家居领域解释评估的方法论空白，为研究人员提供了一个可扩展、真实的评估平台，有助于推进可解释智能系统的用户中心评估
...
### [42] [Decision Support System for Technology Opportunity Discovery: An Application of the Schwartz Theory of Basic Values](https://arxiv.org/abs/2602.11855)
...
TL;DR: 提出结合技术成熟度与人类基本价值理论的新决策框架，用于早期技术机会发现，通过索尼案例验证其可行性


<details>
  <summary>Details</summary>
Motivation: 现有技术机会发现方法在早期开发阶段常忽视终端用户视角，导致技术潜力与市场相关性错位，需要系统整合人类价值视角

Method: 提出整合技术成熟度等级与Schwartz人类基本价值理论的决策支持框架，通过索尼计算机科学实验室的探索性工作坊分析四项真实技术案例

Result: 发现两个一致模式：内部专家比消费者识别更广泛的价值景观（愿景差距），成功技术展现更广泛的人类价值关联（价值广度）

Conclusion: 该框架为早期研发决策提供实用工具，建立价值理论与创新成果的理论联系，强调以价值为中心评估作为以人为本技术机会发现的基础
...
### [43] [Who Does What? Archetypes of Roles Assigned to LLMs During Human-AI Decision-Making](https://arxiv.org/abs/2602.11924)
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在高风险决策领域应用增加，需要批判性反思人类与LLM在决策中的角色分配和交互方式的社会技术因素，以理解不同协作模式对决策结果的影响。

Method: 1) 提出人类-LLM原型概念；2) 通过范围性文献综述和主题分析113篇LLM支持决策论文，识别17种原型；3) 在真实临床诊断案例中评估不同原型对LLM输出和决策结果的影响；4) 分析原型间的设计权衡。

Result: 识别出17种人类-LLM协作决策原型，实证研究表明原型选择显著影响LLM输出和决策结果，揭示了决策控制、社会层级、认知强制策略和信息需求等关键设计维度。

Conclusion: 人类-LLM交互原型的选择对LLM输出和决策有重要影响，为人类-AI决策系统设计者提供了关键的风险考量框架和设计选择指导。
...
### [44] [Wisdom of the LLM Crowd: A Large Scale Benchmark of Multi-Label U.S. Election-Related Harmful Social Media Content](https://arxiv.org/abs/2602.11962)
*Qile Wang,Prerana Khatiwada,Carolina Coimbra Vieira,Benjamin E. Bagozzi,Kenneth E. Barner,Matthew Louis Mauriello*

Main category: cs.HC

TL;DR: USE24-XD是一个包含近10万条推文的大规模数据集，收集自2024年美国总统选举周期，采用六种大型语言模型进行五类有害内容标注，并通过众包验证，揭示了标注者政治背景对内容判断的主观性影响。


<details>
  <summary>Details</summary>
Motivation: 选举错误信息和有害政治内容的传播对民主完整性构成严重威胁。早期检测有害内容对于理解和缓解其后续传播至关重要。需要大规模、高质量的数据集来支持相关研究。

Method: 1. 从X平台收集近10万条2024年美国总统选举周期推文，包含时空元数据；2. 使用六种大型语言模型对帖子进行五类标注：阴谋论、煽情主义、仇恨言论、猜测和讽刺；3. 通过众包验证LLM标注，并与人工标注者进行基准比较；4. 采用群体智慧方法聚合LLM标注，构建稳健的多标签数据集；5. 分析人工标注者人口统计学特征对标注行为的影响。

Result: 1. 评估者间信度分析显示LLM与人类标注者具有可比的一致性模式，LLM表现出更高的内部一致性；2. LLM在猜测类别上达到0.90的召回率；3. 60%的帖子至少获得一个标签；4. 研究发现人工标注者的政治意识形态和党派归属显著影响有害内容的判断，揭示了系统性的主观性来源。

Conclusion: USE24-XD数据集为选举有害内容检测研究提供了宝贵资源。LLM能够以较低成本实现大规模内容标注，与人类标注者表现相当。研究强调了内容判断中政治背景带来的主观性，为未来研究提供了重要启示。数据集已公开发布以支持相关研究。
...
### [45] [Task-Oriented Direct-to-Cell Satellite Communications for Closed-Loop Operations](https://arxiv.org/abs/2602.11195)
*Daohong Shen,Wei Feng,Yunfei Chen,Jinxia Cheng,Dapeng Wang*

Main category: eess.SP

TL;DR: 该论文提出了一种面向任务的卫星直接到单元通信系统设计方法，采用感知-通信-计算-控制闭环模型，从熵的角度分析系统性能，相比传统通信导向方案在任务控制方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 机器人导向的D2C卫星通信在自主操作中更注重任务完成而非单纯数据传输，这要求评估系统各阶段性能并进行集成优化。传统通信导向方案无法满足自主操作的特殊需求。

Method: 采用感知-通信-计算-控制闭环模型，从熵的角度分析系统性能，开发面向任务的系统设计方法。将复杂闭环系统分解为细粒度功能结构，研究协同感知、协同计算和协同控制等关键挑战。

Result: 通过案例研究比较了提出的面向任务方案与传统通信导向方案，结果表明所提方法在任务控制方面表现更优。同时识别了实际实施中需要解决的若干开放研究挑战。

Conclusion: 面向任务的D2C卫星通信系统设计方法能够更好地满足自主操作需求，通过SC3闭环模型和熵分析实现了系统性能优化，为机器人导向的卫星通信提供了有效框架。
...
### [46] [Position-Aware Self-supervised Representation Learning for Cross-mode Radar Signal Recognition](https://arxiv.org/abs/2602.11196)
*Hongyang Zhang,Haitao Zhang,Yinhao Liu,Kunjie Lin,Yue Huang,Xinghao Ding*

Main category: eess.SP

TL;DR: RadarPos：一种位置感知的自监督框架，用于开放电磁环境中的雷达信号识别，通过脉冲级时序动态建模提升位置关系捕获能力，在长尾分布下评估跨模式识别性能。


<details>
  <summary>Details</summary>
Motivation: 开放电磁环境中雷达信号识别面临挑战，现有方法常忽略脉冲序列中的位置关系，难以捕捉时序语义依赖。需要一种能有效建模位置关系、无需复杂增强或掩码的自监督方法。

Method: 提出RadarPos框架，利用脉冲级时序动态进行位置感知的自监督学习，不依赖复杂数据增强或掩码重建，相比对比学习或掩码重建方法能更好地建模位置关系。

Result: 实验结果表明，该方法在长尾分布下的跨模式雷达信号识别任务中表现出增强的判别性和鲁棒性，在真实电磁环境中具有实际应用价值。

Conclusion: RadarPos框架通过位置感知的自监督学习有效提升了雷达信号识别的性能，特别是在开放电磁环境和长尾分布场景下，展示了良好的适应性和泛化能力。
...
### [47] [Target Induced Angle Grid Regularized Estimation for Ghost Identification in Automotive Radar](https://arxiv.org/abs/2602.11441)
*Junho Kweon,Vishal Monga*

Main category: eess.SP

TL;DR: 提出TIGRE算法，通过物理启发的正则化增强多径环境下汽车雷达的鬼目标识别能力，特别提升低信噪比条件下的性能


<details>
  <summary>Details</summary>
Motivation: 汽车雷达在真实驾驶环境中常因护栏等表面反射产生多径传播，导致出现位置错误的鬼目标，影响自动驾驶导航。现有MP-IAA方法在低信噪比环境下效果下降，需要更鲁棒的解决方案

Method: 提出TIGRE算法，引入物理启发的正则化器捕捉多径传播的结构模式，将其融入估计成本形成新的损失函数，迭代最小化该正则化损失，并证明该正则化器渐近地强制执行DOA/DOD网格上的L0稀疏性

Result: 数值实验表明，该方法在各种多径场景下显著提升了角度网格估计质量，特别是在低信噪比环境中，为后续鬼目标识别提供了更可靠的基础

Conclusion: TIGRE算法通过物理启发的正则化增强了多径环境下汽车雷达鬼目标识别的鲁棒性，特别在低信噪比条件下表现优异，为解决自动驾驶中的虚假检测问题提供了有效方案
...
### [48] [Around-the-corner Radar Sensing Using Reconfigurable Intelligent Surface](https://arxiv.org/abs/2602.11471)
*Kainat Yasmeen,Debidas Kundu,Shobha Sundar Ram*

Main category: eess.SP

TL;DR: 利用可重构智能表面(RIS)增强非视距条件下的雷达感知能力，通过RIS将入射波束定向到NLOS区域，成功捕获人类行走的微多普勒特征


<details>
  <summary>Details</summary>
Motivation: 传统绕角雷达(ACR)在非视距条件下通过墙壁多径反射检测目标，但多径信号强度低导致检测指标较弱，需要提升NLOS场景下的雷达感知性能

Method: 应用可重构智能表面(RIS)技术，通过战略部署RIS将入射波束定向到非视距区域，在5.5GHz频段进行实验验证

Result: 实验结果表明，通过RIS的战略部署，现在能够在非视距条件下成功捕获人类行走运动的微多普勒特征

Conclusion: RIS技术能够显著改善绕角雷达在非视距条件下的感知性能，为安全监控和汽车预警系统等应用提供了新的解决方案
...
### [49] [Radar Sensing using Dual-Beam Reconfigurable Intelligent Surface](https://arxiv.org/abs/2602.11473)
*Kainat Yasmeen,Shobha Sundar Ram,Debidas Kundu*

Main category: eess.SP

TL;DR: 该论文研究了可重构智能表面(RIS)在墙角雷达感知中的应用，分析了单元相位调谐的实用考虑对RIS性能的影响，比较了模拟相位调谐和1比特量化相位调谐两种配置的性能。


<details>
  <summary>Details</summary>
Motivation: 墙角雷达感知可以利用墙壁的多径散射探测被遮挡的目标，但传统雷达只能探测镜面角度的非合作目标。RIS通过可调谐单元能够增强墙角雷达的覆盖范围，将波束导向非镜面角度，但实际应用中单元相位调谐的实用性问题需要研究。

Method: 研究两种RIS配置：1)基于理论模拟相位调谐实现理想单波束模式；2)基于低复杂度1比特量化单元相位调谐产生对称双波束。通过仿真和测量将两种RIS配置与相似尺寸的金属板进行基准比较。

Result: 通过雷达散射截面(RCS)分析评估了两种RIS配置的性能。模拟相位调谐配置能够实现理想化的单波束模式，而1比特量化相位调谐配置产生对称双波束模式。两种配置都与金属板进行了性能对比。

Conclusion: 单元相位调谐的实用考虑对RIS性能有重要影响。模拟相位调谐可实现理想波束模式但复杂度高，而1比特量化相位调谐虽然产生对称双波束但复杂度低。研究为墙角雷达感知中RIS的实际应用提供了重要指导。
...
### [50] [C-POD: An AWS Cloud Framework for Edge Pod Automation and Remote Wireless Testbed Sharing](https://arxiv.org/abs/2602.11585)
...
Method: 利用公共云资源和边缘pod构建云原生框架，开发支持工具包（包括容器化边缘环境、持久SSH隧道、稳定图形界面），在AWS云上原型部署，集成两个无线测试平台（RF信号生成和5G(B)通信）。

Result: 成功演示了C-POD的关键特性：云辅助边缘pod部署自动化、弹性计算资源管理、实验可观测性，通过集成两个无线测试平台验证了框架的有效性。

Conclusion: C-POD框架能够有效降低无线测试平台的访问门槛，实现分布式测试平台的灵活、可扩展共享，为研究人员提供便捷的远程实验环境。
...
### [51] [Beamforming Gain Maximization for Fluid Reconfigurable Intelligent Surface: A Minkowski Geometry Approach](https://arxiv.org/abs/2602.11654)
*Hong-Bae Jeon*

Main category: eess.SP

TL;DR: 本文提出了一种用于流体可重构智能表面辅助下行链路系统的波束赋形增益最大化方法，通过交替优化框架解决BS波束赋形器与FRIS端口选择和离散相位分配的耦合问题。


<details>
  <summary>Details</summary>
Motivation: FRIS辅助下行链路系统中，每个活动端口应用有限分辨率单位模相位（选自离散码本），导致多天线基站波束赋形器与组合FRIS端口选择及离散相位分配相耦合，形成高度非凸混合离散优化问题。

Method: 开发交替优化框架：在BS端采用闭式最大比传输更新，在FRIS端采用最优配置更新。关键步骤是通过Minkowski几何重构FRIS码本叠加，凸化可行反射和集并利用支撑函数恒等式，将FRIS子问题转化为方向参数的一维最大化。对于每个方向，通过端口方向评分、Top-Mo端口选择和最优码字分配构造最优配置。针对重要的正Mp边形移相器码本，推导闭式评分表达式并建立支撑函数的分段光滑结构，实现无需穷举角度扫描的有限临界角搜索。

Result: 仿真结果表明，所提框架始终优于基准方法，在穷举搜索验证中实现接近最优的波束赋形增益，通过支撑函数最大化准确识别最优方向，并在少数AO迭代内快速收敛。

Conclusion: 本文提出的交替优化框架有效解决了FRIS辅助系统中波束赋形增益最大化的混合离散优化问题，通过Minkowski几何重构和支撑函数分析实现了高效的最优配置，为有限分辨率相移器码本提供了理论保证和实用算法。
...
### [52] [On the Maintainability of Pinching-Antenna Systems: A Failure-Repair Perspective](https://arxiv.org/abs/2602.11784)
*Chongjun Ouyang,Hao Jiang,Zhaolin Wang,Yuanwei Liu,Zhiguo Ding*

Main category: eess.SP

TL;DR: 提出统一分析框架评估夹持天线系统(PASS)的可维护性，通过马尔可夫链建模波导故障与修复，推导非零速率概率和中断概率，证明分段波导系统(SWAN)相比传统PASS具有可维护性优势。


<details>
  <summary>Details</summary>
Motivation: 夹持天线系统(PASS)通过沿介质波导优化放置夹持天线实现无线信道重构，但波导故障会影响系统性能。需要建立分析框架来量化系统的可维护性，特别是比较传统单长波导PASS与分段波导系统(SWAN)的性能差异。

Method: 建立统一分析框架：将波导寿命和修复时间建模为指数分布随机变量，用两状态连续时间马尔可夫链描述波导工作状态。将波导状态随机性纳入传输速率，使用非零速率概率(PNR)和中断概率(OP)表征系统可维护性。应用于传统PASS和SWAN两种架构，在段切换(SS)和段聚合(SA)两种操作协议下推导PNR和OP的闭式表达式。

Result: 理论证明：基于SS和SA的SWAN都比传统PASS获得更高的PNR和更低的OP，证实了分段化的可维护性优势。数值结果表明：1) SWAN相对于传统PASS的可维护性增益随分段数增加而增加；2) SA比SS提供更强的可维护性。

Conclusion: 提出的统一分析框架成功量化了PASS系统的可维护性，证明了分段波导架构(SWAN)在可维护性方面的显著优势，特别是段聚合(SA)协议表现最佳，为无线信道重构系统的可靠设计提供了理论指导。
...
### [53] [EqDeepRx: Learning a Scalable MIMO Receiver](https://arxiv.org/abs/2602.11834)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于机器学习的接收机算法存在两个主要问题：1) 随着空间复用阶数增加，复杂度呈非线性增长；2) 缺乏可解释性和泛化能力。需要一种既保持高性能又具备实用性的MIMO接收机解决方案。

Method: EqDeepRx采用混合架构：保留传统信道估计，增加轻量级DenoiseNN进行频域平滑；使用并行LMMSE和RZF均衡器降低DetectorNN输入维度；核心是共享权重的DetectorNN，独立处理每个空间流；最后通过紧凑的DemapperNN生成比特对数似然比用于信道解码。

Result: 在5G/6G兼容的端到端仿真中，EqDeepRx在多种信道场景、导频模式和小区间干扰条件下，相比传统基线方法表现出更好的误码率和频谱效率，同时保持低复杂度推理，并支持不同MIMO配置而无需重新训练。

Conclusion: EqDeepRx成功解决了ML-based接收机的可扩展性和可解释性问题，通过精心设计的混合架构实现了性能提升和复杂度控制的平衡，为实际MIMO系统部署提供了可行的深度学习辅助接收机方案。
...
TL;DR: 该论文研究了在堆叠智能超表面通信系统中引入结构灵活性的潜力，提出了两种灵活架构：距离自适应SIM和堆叠灵活智能超表面，通过联合优化元原子位置、响应和发射波束形成器来最大化系统和速率。


<details>
  <summary>Details</summary>
Motivation: 传统SIM设计采用刚性平面层和固定层间距，限制了传播几何结构，随着层数增加会导致性能饱和。因此需要探索结构灵活性以突破这些限制。

Method: 提出了两种灵活SIM架构：DSIM（优化层间距）和SFIM（每层完全可变形）。开发了结合梯度投影、惩罚方法和逐次凸近似的交替优化框架，联合设计元原子位置、响应和发射波束形成器。

Result: 扰动分析显示两种灵活设计的灵活性增益近似线性随变形范围增长，其中SFIM增长更快。仿真表明灵活SIM设计能缓解随层数增加的性能饱和问题，相比刚性SIM显著节省发射功率。

Conclusion: 在SIM通信系统中引入结构灵活性是有效的，能够突破传统刚性设计的限制，提高系统性能并降低功耗，为未来可重构无线通信系统提供了新方向。
...
### [55] [Continuous and Discrete-Time Filters: A Unified Operational Perspective](https://arxiv.org/abs/2602.12198)
*Luca Giangrande*

Main category: eess.SP

TL;DR: 该论文提供了连续时间和离散时间线性时不变系统的统一教程处理，强调其共享的动态结构和区分其实现的物理约束，从信号处理和电路实现的操作视角重新审视基本概念。


<details>
  <summary>Details</summary>
Motivation: 弥合形式化数学描述与混合模拟-数字环境中可靠系统设计所需直觉之间的差距，为工程师提供从操作视角理解系统动态的实用框架。

Method: 使用一阶系统作为最小但表达性强的框架，在拉普拉斯域和Z域中展示积分、微分、滤波和延迟的实现；从信号处理和电路实现的操作视角重新审视传递函数、极点和零点、脉冲响应和稳定性等基本概念。

Result: 建立了连续时间和离散时间系统之间的统一理解框架，阐明了因果性、带宽限制、采样效应和离散时间表示固有近似误差等关键概念，为混合模拟-数字系统设计提供了实用指导。

Conclusion: 通过操作视角的统一处理，该教程成功连接了形式数学描述与工程直觉，为混合模拟-数字环境中的系统设计提供了更直观和实用的理解框架。
